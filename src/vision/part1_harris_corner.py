...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    kernel_x = torch.from_numpy(SOBEL_X_KERNEL).unsqueeze(0).unsqueeze(0)
    kernel_y = torch.from_numpy(SOBEL_Y_KERNEL).unsqueeze(0).unsqueeze(0)
    img = torch.from_numpy(image_bw).float().unsqueeze(0).unsqueeze(0)
    Ix_tensor = torch.nn.functional.conv2d(img, kernel_x, padding=1)
    Iy_tensor = torch.nn.functional.conv2d(img, kernel_y, padding=1) # what is the padding supposed to be
    Ix, Iy = Ix_tensor.squeeze().numpy(), Iy_tensor.squeeze().numpy()
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return Ix, Iy
...
def compute_harris_response_map(
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    sx2, sy2, sxsy = second_moments(image_bw=image_bw, ksize=ksize, sigma=sigma)
    R = np.ones((len(image_bw), len(image_bw[0])))
    for i in range(len(image_bw)):
        for j in range(len(image_bw[0])):
            R[i][j] = sx2[i][j] * sy2[i][j] - (sxsy[i][j] **2)
            R[i][j] -= alpha * ((sx2[i][j] +sy2[i][j]) ** 2)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def nms_maxpool_pytorch(
    ...
    med = np.median(R)
    R[R < med] = 0
    pooled_tensor_r = maxpool_numpy(R, ksize)
    mask = np.ones((len(R), len(R[0])))
    for i in range(len(R)):
        for j in range(len(R[0])):
            if pooled_tensor_r[i][j] != R[i][j]:
                mask[i][j] = 0
    res = mask * R

    sortedIndices = np.argsort(res.flatten())[::-1]
    x, y, confidences = [],[],[]
    for i in range(k):
        xIdx, yIdx =  sortedIndices[i] % res.shape[1], sortedIndices[i] // res.shape[1]
        x.append(xIdx)
        y.append(yIdx)
        confidences.append(res[yIdx][xIdx])
...
def get_harris_interest_points(
    ...
    # Step 1: Compute the Harris response map
    harris_response = compute_harris_response_map(image_bw, ksize=7, sigma=5, alpha=0.05)
    harris_response = (harris_response - np.min(harris_response)) / (np.max(harris_response) - np.min(harris_response))
    x, y, c = nms_maxpool_pytorch(harris_response, k, ksize=7)
